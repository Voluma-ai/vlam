...
const label = (strategy: ReturnType<typeof demoSortStrategy>) =>
  typeof strategy === 'string' ? strategy : strategy.exactDepth ? 'exact' : 'radix';
...
describe('demo sorting defaults', () => {
  it.each(['castle.lcc', '/remote/castle.LCC2', 'https://example.test/castle.lcc2?token=x#view'])(
    ...
    (scene) => expect(label(demoSortStrategy(scene, hd))).toBe('radix'),
  );
  ...
  it('falls back to the format policy for an unrecognized override', () => {
    expect(label(demoSortStrategy('castle.lcc2', { ...hd, override: 'typo' }))).toBe('radix');
  });
});
