...
function solar(instant: number, latitude: number, longitude: number, format: Intl.DateTimeFormat) {
  ...
  const day =
    Math.floor((Date.UTC(c.year, c.month - 1, c.day) - Date.UTC(c.year, 0, 1)) / 86400000) + 1;
  const days = new Date(Date.UTC(c.year, 1, 29)).getUTCMonth() === 1 ? 366 : 365;
  const gamma =
    ((2 * Math.PI) / days) * (day - 1 + (c.hour + c.minute / 60 + c.second / 3600 - 12) / 24);
  const equation =
    229.18 *
    (0.000075 +
  ...
  const solarMinutes =
    c.hour * 60 + c.minute + c.second / 60 + equation + 4 * longitude - 60 * offsetHours;
...
export function createRelightClock() {
  ...
  const defaultCity =
    requestedCity && requestedCity in presets
  ...
  lat.max = '90';
  lat.step = 'any';
  lat.setAttribute('aria-label', 'Latitude');
  const lon = document.createElement('input');
  lon.type = 'number';
  lon.min = '-180';
  lon.max = '180';
  lon.step = 'any';
  lon.setAttribute('aria-label', 'Longitude');
  const tz = document.createElement('input');
  tz.type = 'text';
  tz.setAttribute('aria-label', 'IANA time zone');
  const zoneSuggestions = document.createElement('datalist');
  zoneSuggestions.id = 'relight-time-zones';
  const zones =
    typeof Intl.supportedValuesOf === 'function' ? Intl.supportedValuesOf('timeZone') : [];
  for (const value of new Set([
    'UTC',
    ...Object.values(presets).map((preset) => preset.zone),
    ...zones,
  ])) {
  ...
  heading.step = 'any';
  heading.value = '0';
  heading.setAttribute('aria-label', 'North heading in degrees');
  const field = (label: string, input: HTMLElement): HTMLLabelElement => {
  ...
  const setCivil = (day: string, minute: number): void => {
    if (!day) return;
    instant = resolveCivil(day, minute, format);
    invalidate();
  };
  ...
  lat.onchange = () => {
    ...
    if (Number.isFinite(v)) latitude = Math.max(-90, Math.min(90, v));
    city.value = 'Custom';
    invalidate();
  };
  lon.onchange = () => {
    const v = Number(lon.value);
    if (Number.isFinite(v)) longitude = Math.max(-180, Math.min(180, v));
    city.value = 'Custom';
    invalidate();
  };
  tz.onchange = () => {
    try {
      const next = formatter(tz.value);
      next.format(instant);
      zone = tz.value;
      format = next;
      invalidate();
      ...
      tz.setCustomValidity('Enter a valid IANA time zone');
      tz.reportValidity();
    }
  };
  heading.oninput = () => {
    north = Number(heading.value) || 0;
    invalidate();
  };
  sync();
  return {
    ...
    tick() {
      ...
      if (playing && !panel.hidden) {
        ...
        if (elevation < 0 && nightEnd <= instant) {
          ...
          while (
            next < instant + 172800000 &&
            solar(next, latitude, longitude, format).elevation < 0
          )
            next += 300000;
          nightEnd = next;
        ...
        instant = Math.min(
          nightEnd > instant ? nightEnd : Infinity,
          instant + dt * (elevation < 0 ? nightSpeed : 360000),
        );
        sync();
      }
