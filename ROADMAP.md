...
- **Surface-aware brush and selection precision** — extend the existing
  ...
  **Acceptance:** inspect thin surfaces, foreground/background boundaries,
  transformed meshes, and large captures; protect async picks from camera,
  scene, tool, and pointer lifecycle changes. Preserve streamed edits across
  residency changes and provide backend-appropriate behavior.
