...
| Do not overlap GPU counting sorts | One pass owns `splatIndex` until `onSubmittedWorkDone`. A second dispatch while the first is in flight lets the older scatter finish over a newer streamed cut (Tempel/LCC2 white-blob flashes, worse with relighting). Hold while that pass is in flight. Coalesce **content** so it re-sorts when the buffer is free; camera-only motion keeps cadence. Delay classic LOD commits until the buffer is free, except the hidden startup hold and first cover (otherwise `isStreaming` never clears). Page-table publication already retains the previous draw count. Generic `SplatMesh` / `StaticLodSplatMesh` keep the previous `instanceCount` and GPU `sourceIndex` until that matching sort can run; changing them sooner draws a stale permutation (removed splats linger, live ones vanish). |
