...
1. [Getting started](getting-started.md): install, WebGPU renderer setup,
   first capture, the per-frame update loop, cleanup, and the WebGL2 fallback.
2. [Terminology](terminology.md): capture, splat data, mesh, source, scene,
   budgets, and LOD words.
3. [Loading captures](loading-scenes.md): `loadSplatData`/`loadSplatDataFile`, format
   auto-detection and subpaths, structured errors, progress, cancellation.
4. [Streaming & LOD](streaming-and-lod.md): `StreamedSplatMesh` for captures
   larger than GPU memory: budgets, `BudgetGovernor`, local folders,
   collision meshes, the environment tile.
5. [Multi-mesh budgets](multi-mesh-budgets.md): one shared splat
   budget across a main mesh and several additional meshes, reweighted from the
...
6. [Unified rendering](unified-rendering.md): depth-correct compositing of
   fully loaded and streamed sources in one WebGPU draw, with per-source
